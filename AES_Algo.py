    ...
    for element in range(0,len(hexaChunks)):
        ...
        for element1 in range(0,len(hexaChunks[element])):
            for element2 in range(0,len(hexaChunks[element][element1])):
                ...
                if i<4:
                    list4.append(hex(int(sBox[tempo][tempo1]))[2:])
                    i+=1
                else:
                    i=0
                    list4=[]
...
def shiftRow(firstStageMassage1):
    #this function's rule is to shift the matrixs 
    firstStageMassage=deque(firstStageMassage1)
    shiftedMatrixs=[]
    for i in range(0,len(firstStageMassage)):
        shifted16List=[]
        shifted16List.append(firstStageMassage[i][0])
        shifted16List.append(firstStageMassage[i][1][-1:]+firstStageMassage[i][1][:-1])
        shifted16List.append(firstStageMassage[i][2][-2:]+firstStageMassage[i][2][:-2])
        shifted16List.append(firstStageMassage[i][3][-3:]+firstStageMassage[i][3][:-3])
...
by2=[[0x00,0x02,0x04,0x06,0x08,0x0a,0x0c,0x0e,0x10,0x12,0x14,0x16,0x18,0x1a,0x1c,0x1e],
...
by3=[[0x00,0x03,0x06,0x05,0x0c,0x0f,0x0a,0x09,0x18,0x1b,0x1e,0x1d,0x14,0x17,0x12,0x11],
                ...
                for t in range(0,len(shiftedMatrix[x])): #iterate through raws of shiftedMatrix
                    if len(shiftedMatrix[x][t][y])==1:
                        ...
                        if C[i][t] == 1:
                            ...
                            result = hex(int(result,base=16) ^ int(hex(int(by2[0][tempo]))[2:],base=16))
                        elif C[i][t] == 3:
                            #print(hex(int(result,base=16)),hex(int(hex(int(by3[tempo][tempo1]))[2:].upper(),base=16)))
                            result = hex( int(result,base=16) ^ int(hex(int(by3[0][tempo]))[2:],base=16))
                    else:
                        tempo=int(shiftedMatrix[x][t][y][0],16)
                        ...
                        if C[i][t] == 1:
                            ...
                            #print(hex(int(result,base=16)),hex(int(hex(int(by2[tempo][tempo1]))[2:].upper(),base=16)))
                            result = hex(int(result,base=16) ^ int(hex(int(by2[tempo][tempo1]))[2:],base=16))
                        elif C[i][t] == 3:
                            #print(hex(int(result,base=16)),hex(int(hex(int(by3[tempo][tempo1]))[2:].upper(),base=16)))
                            result = hex( int(result,base=16) ^ int(hex(int(by3[tempo][tempo1]))[2:],base=16))
