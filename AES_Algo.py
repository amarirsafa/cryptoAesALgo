...
clairMassage = input("Enter you massage here!:")

def getTextReady(clairMassage):
# this functions main role is to take the text and to transform it into chunks of text, meaning to list of 16bytes.
    ...
    if len(clairMassage)<16:
        ...
        if len(chunks[-1]) < 16:
            tempo=16-len(chunks[-1])
            chunks[-1]=chunks[-1]+tempo*"0"
        return chunks
    else :
        return clairMassage
...
def convertChunksToHexa(clairMassageChuncks):
    #this functions main rule is to turn clairMassageChunks into hexa format
...
def transpose(hexaChunks1):
    ...
    hexaChunk16=[]
    hexaChunk4=[]
    for i in range(0,len(hexaChunks1)):
        for y in range(0,len(hexaChunks1[i])):
            for t in range(0,len(hexaChunks1[i][y])):
                hexaChunk4.append(hexaChunks1[i][t][y])
            hexaChunk16.append(hexaChunk4)
            hexaChunk4=[]
        hexaChunks.append(hexaChunk16)
        hexaChunk16=[]
    return hexaChunks

hexaChunks = transpose(hexaChunks1)
for index in range(0,len(hexaChunks)):
    print("Matrix number ",index+1)
    for element in range(len(hexaChunks[index])):
        print(hexaChunks[index][element],"\n")
        ...
        for element1 in range(len(hexaChunks[element])):
            for element2 in range(0,len(hexaChunks[element][element1])):
                ...
                if len(list4)==4:
                    ...
                    continue
        matrixs.append(list16)
        list16=[]
    return matrixs

firstStageMassage=runThroughSBox(hexaChunks)
for v in range(0,len(firstStageMassage)):
    print("Matrix number (after sbox table )",v+1)
    for j in range(0,len(firstStageMassage[v])):
        print(firstStageMassage[v][j])
print("--------------------------------stage two--------------------------------")
print("----------------------This is the stage of shit row----------------------")
def shiftRow(firstStageMassage):
    #this function's rule is to shift the matrixs 
    shiftedMatrixs=[]
    for i in range(0,len(firstStageMassage)):
        shifted16List=[]
        shifted16List.append(firstStageMassage[i][0])
        shifted16List.append(np.roll(firstStageMassage[i][1], -1, axis=0))
        shifted16List.append(np.roll(firstStageMassage[i][2], -2, axis=0))
        shifted16List.append(np.roll(firstStageMassage[i][3], -3, axis=0))
        shiftedMatrixs.append(shifted16List)
    return shiftedMatrixs

shiftedMatrix=shiftRow(firstStageMassage)
for v in range(0,len(shiftedMatrix)):
    print("Matrix number (after shift row )",v+1)
    for j in range(0,len(shiftedMatrix[v])):
        print(shiftedMatrix[v][j])

print("--------------------------------stage three--------------------------------")
print("----------------------This is the stage of mixColumn----------------------")
C = [[2,3,1,1],
    [1,2,3,1],
    [1,1,2,3],
    [3,1,1,2]]
def mixColumn(shiftedMatrix):
    ...
    mixColumn=[]
    n=0
    for i in range(0,len(shiftedMatrix)):
        for j in range(0,len(shiftedMatrix[i])):
            tempo=0
            m=0
            for t in range(0,len(C)):
                tempo=tempo^C[t][m]*shiftedMatrix[i][j][m][n]
                m+=1
            n+=1
            mixColumn4.append(tempo)
